package com.multi.tenants.api.jwt;
...
@Slf4j
@Data
public class ITzBaseJwt implements Serializable {
    ...
    public static ITzBaseJwt decode(String input){
        ITzBaseJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM,11);
            if(items.length >= 10){
                result = new ITzBaseJwt();
