package com.multi.tenants.api.config;

import com.multi.tenants.api.dto.AccountForTokenDto;
import com.multi.tenants.api.model.Permission;
import com.multi.tenants.api.utils.ZipUtils;
...
public class CustomTokenEnhancer implements TokenEnhancer {
    ...
    public OAuth2AccessToken enhance(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
        ...
        if(grantType == null){
            ...
            if(grantType.equals(SecurityConstant.GRANT_TYPE_USER) ){
                ...
                String phone = authentication.getOAuth2Request().getRequestParameters().get("phone");
                String deviceId = authentication.getOAuth2Request().getRequestParameters().get("deviceId");
                String deviceSerial = authentication.getOAuth2Request().getRequestParameters().get("deviceSerial");

                additionalInfo = getAdditionalEmployeeInfo(phone, grantType, deviceId, deviceSerial);
            }
        }
    ...
    private Map<String, Object> getAdditionalEmployeeInfo(String phone, String grantType, String deviceId, String deviceSerial) {
        Map<String, Object> additionalInfo = new HashMap<>();
        AccountForTokenDto a = getUserByPhone(phone);
        ...
        if (a != null) {
            ...
            additionalInfo.put("grant_type", grantType == null ? SecurityConstant.GRANT_TYPE_PASSWORD : grantType);
            additionalInfo.put("tenant_info", tenantId);
            additionalInfo.put("device_id", deviceId);
            additionalInfo.put("device_serial", deviceSerial);
            String DELIM = "|";
            String additionalInfoStr = ZipUtils.zipString(userId + DELIM
                + storeId + DELIM
                + kind + DELIM
    ...
    public AccountForTokenDto getAccountByUsername(String username) {
        try {
            String query = "SELECT id, kind, username, email, full_name, is_super_admin " +
                    "FROM db_tenants_master_account WHERE username = ? and status = 1 limit 1";
            log.debug(query);
    ...
    public AccountForTokenDto getUserByPhone(String phone) {
        try {
            String query = "SELECT id, kind, username, email, full_name, is_super_admin " +
                    "FROM db_tenants_master_account WHERE phone = ? and status = 1 limit 1";
            log.debug(query);
    ...
    public List<Permission> getListPermissionByUserKind(Integer kind) {
        try{
            String query = "select * " +
                    "from db_tenants_master_permission p " +
                    "join db_tenants_master_permission_group g on p.id = g.permission_id " +
                    "join db_tenants_master_account a on a.group_id = g.group_id " +
                    "where a.kind = ?";
