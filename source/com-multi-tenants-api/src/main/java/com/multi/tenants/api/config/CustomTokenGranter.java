    ...
    protected OAuth2AccessToken getAccessToken(ClientDetails client, TokenRequest tokenRequest) {
        ...
        try {
            if(SecurityConstant.GRANT_TYPE_USER.equalsIgnoreCase(tokenRequest.getGrantType())){
                ...
                String phone = tokenRequest.getRequestParameters().get("phone");
                String serial = tokenRequest.getRequestParameters().get("serial");
                return userService.getAccessTokenForEmployee(client, tokenRequest, password, phone, serial, this.getTokenServices());
            } else{
